 ...
 */
public class Socket5 implements Runnable {
     ...
     */
    protected Socket5(Socket socket, boolean openSock5, String user, String pwd) {
        this.socket = socket;
...
//                } else
                if ((openSock5 && 0x05 == protocol)) {// 如果开启代理5，并以socks5协议请求
...
//                    log("not socks proxy : %s  openSock4[] openSock5[]", tmp[0], openSock4, openSock5);
                    log("not socks proxy : %s  openSock4[] openSock5[]", tmp[0], openSock5);
                }
    ...
    public static void startServer(int port, boolean openSock4, boolean openSock5, String user, String pwd)
        ...
        while (null != (socket = ss.accept())) {
            new Thread(new Socket5(socket, openSock5, user, pwd)).start();
        }
        ss.close();
    }
...
//            pwd = args[i].trim();
//        }
        Socket5.startServer(port, openSock4, openSock5, user, pwd);
    }
}
